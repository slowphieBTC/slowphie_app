...
export function SchoolGlossary() {
  ...
  const filtered = useMemo<GlossaryTerm[]>(() => {
    ...
    if (q) return GLOSSARY_TERMS.filter(t => t.term.toLowerCase().includes(q) || t.definition.toLowerCase().includes(q));
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '0.75rem', flexWrap: 'wrap' }}>
            <Link to="/school" style={{ fontSize: '0.8rem', color: '#F7931A', textDecoration: 'none', fontWeight: 600 }}>← School</Link>
            <h1 className="text-3xl md:text-4xl font-extrabold text-white" style={{ margin: 0 }}>
              <span className="text-gradient">Glossary</span>
            </h1>
          </div>
          <p style={{ color: '#475569', margin: 0, fontSize: '0.88rem' }}>All key terms from the OPNet &amp; Bitcoin DeFi course, defined in plain English.</p>
        </motion.div>
        ...
        <div style={{ position: 'relative', marginBottom: '0.625rem' }}>
          ...
          <input
            ...
            placeholder="Search 40+ Bitcoin DeFi terms..."
        ...
        <p style={{ color: '#334155', fontSize: '0.78rem', marginBottom: '1.5rem' }}>
          Showing <span style={{ color: '#F7931A', fontWeight: 700 }}>{filtered.length}</span> of {GLOSSARY_TERMS.length} terms
          {search && <> matching <em style={{ color: '#F7931A' }}>&ldquo;{search}&rdquo;</em></>}
          {(search || activeLetter) && <button onClick={() => { setSearch(''); setActiveLetter(null); }} style={{ marginLeft: '0.5rem', background: 'none', border: 'none', color: '#EF4444', cursor: 'pointer', fontSize: '0.75rem', fontWeight: 600, padding: 0 }}>Clear</button>}
        </p>
        ...
        <div style={{ display: 'flex', gap: '1.75rem', alignItems: 'flex-start' }}>
          ...
          <div style={{ flexShrink: 0, width: '40px' }}>
            <div style={{ position: 'sticky', top: '6rem', display: 'flex', flexDirection: 'column', gap: '2px' }}>
              {allLetters.map(l => (
                <button key={l} onClick={() => jump(l)} style={{
                  width: '38px', height: '28px', borderRadius: '7px',
                  border: (activeLetter === l && !search) ? '1px solid rgba(247,147,26,0.4)' : '1px solid transparent',
                  background: (activeLetter === l && !search) ? 'rgba(247,147,26,0.12)' : activeL.has(l) ? 'rgba(255,255,255,0.03)' : 'transparent',
                  color: (activeLetter === l && !search) ? '#F7931A' : activeL.has(l) ? '#475569' : '#1e293b',
                  fontWeight: 700, fontSize: '0.66rem', cursor: activeL.has(l) ? 'pointer' : 'default', transition: 'all 0.15s'
                }}>{l}</button>
              ))}
            </div>
          </div>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            {filtered.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '4rem 1rem' }}>
                ...
                <h3 style={{ color: '#4a5568', margin: '0 0 0.5rem' }}>No terms found</h3>
                <p style={{ color: '#334155', fontSize: '0.85rem' }}>Try different keywords.</p>
              </div>
              ...
              <div key={ltr} ref={el => { secRefs.current[ltr] = el; }} style={{ marginBottom: '2.5rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.875rem', marginBottom: '0.875rem' }}>
                  ...
                  <span style={{ fontSize: '0.62rem', color: '#1e293b' }}>{items.length} term{items.length !== 1 ? 's' : ''}</span>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                  {items.map(term => (
                    <div key={term.term}
                      style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '12px', padding: '1rem 1.25rem', transition: 'border-color 0.2s' }}
                    ...
                    >
                      ...
                      <p style={{ color: '#4a5568', fontSize: '0.84rem', lineHeight: 1.7, margin: 0 }}><Hl text={term.definition} q={search} /></p>
                      {term.relatedTerms && term.relatedTerms.length > 0 && (
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.3rem', marginTop: '0.625rem', alignItems: 'center' }}>
                          <span style={{ fontSize: '0.62rem', color: '#334155' }}>See also:</span>
