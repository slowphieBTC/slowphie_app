  ...
  return (
    <div className="relative min-h-[calc(100vh-4rem)] bg-grid flex flex-col items-center justify-center px-6 py-16 overflow-hidden">
      ...
      <div style={{ position: 'absolute', top: '1.25rem', right: '1.25rem', display: 'flex', alignItems: 'center', gap: '0.5rem', zIndex: 10 }}>
        ...
        <a href="https://t.me/SlowphieOfficialPortal" target="_blank" rel="noopener noreferrer"
          style={{ width: '30px', height: '30px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '50%', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.04)', color: '#94a3b8', backdropFilter: 'blur(8px)', transition: 'color 0.2s, border-color 0.2s' }}
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: 'easeOut' }} className="text-center mb-16">
        <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight text-white mb-3">
          {t('home.title')}{' '}<span className="text-gradient">Slowphie</span>
        </h1>
        ...
        {modules.map((mod) => (
          <motion.div key={mod.id} variants={cardVariants} className="glass-card cursor-pointer group p-0" onClick={() => navigate(mod.route)} style={{ '--accent': mod.accent } as React.CSSProperties}>
            <div className="relative overflow-hidden rounded-t-3xl">
              <img src={mod.image} alt={mod.title} title="" draggable={false} className="w-[60%] mx-auto block h-auto -mt-[28px] -mb-[50px] transition-transform duration-500 group-hover:scale-105" />
            ...
            <div className="px-4 py-3">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-lg font-bold text-white group-hover:text-gradient transition-all duration-300">{mod.title}</h2>
                  <p className="text-slate-400 text-sm mt-0.5">{mod.description}</p>
                </div>
                <span className="text-xs font-semibold px-3 py-1.5 rounded-full border transition-all duration-300 whitespace-nowrap ml-3" style={{ color: mod.accent, borderColor: `${mod.accent}44`, background: `${mod.accent}11` }}>
                  {mod.label}
                </span>
              </div>
            </div>
