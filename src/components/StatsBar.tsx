...
export function StatsBar() {
      ...
      {connected ? (
        <>
          <motion.span
            ...
            className="w-1.5 h-1.5 rounded-full bg-green-400 shadow-[0_0_4px_rgba(74,222,128,0.8)] flex-shrink-0"
          />
    ...
    <div className="flex items-center gap-1.5">
      ...
      {latestBlock ? (
        <span className="text-xs text-gray-400">
          Block <span className="text-white font-medium">#{latestBlock.height.toLocaleString()}</span>
        </span>
      ) : (
  ...
  const networkEl = (
    <div className="flex items-center gap-1.5">
      ...
      <span className="text-xs text-gray-600">OPNet · Bitcoin Mainnet</span>
    </div>
  );
